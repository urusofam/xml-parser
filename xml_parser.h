...
inline std::string get_field(const std::string& tag_content, const std::string& field_name) {
    // Ищем начало
...
inline unsigned char hexchar_to_int(const char& c) {
...
inline long long hex_to_decimal(const std::string& hex) {
    try {
...
inline void parse_xml(const std::string& xml_data) {
